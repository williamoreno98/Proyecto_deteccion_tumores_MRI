# ==========================================
# detection_model_RF.py
# Entrenamiento de Random Forest con MLflow
# ==========================================
import os
...
def main():
    ...
    for n_est in param_grid["n_estimators"]:
        for depth in param_grid["max_depth"]:
            ...
            run_result = {"n_estimators": n_est, "max_depth": depth,
                          "accuracy": acc, "f1": f1}
            results_rf.append(run_result)

            # Log en MLflow
            with mlflow.start_run(run_name=f"RF_{n_est}_depth{depth}"):
            ...
            print(f"RF(n_estimators={n_est}, depth={depth}) -> Acc: {acc:.4f}, F1: {f1:.4f}")

    # -----------------------------
    # 5) Mejor modelo en test
    # -----------------------------
    best_run = max(results_rf, key=lambda x: x["accuracy"])
    ...
    with mlflow.start_run(run_name="RF_FINAL_BEST"):
        ...
        mlflow.sklearn.log_model(best_model, "rf_final_model")
