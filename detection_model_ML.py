...
def main():
    ...
    X_tr, X_val, y_tr, y_val = train_test_split(
        X_train_all, y_train_enc, test_size=0.2, random_state=RANDOM_STATE, stratify=y_train_enc
    )
    ...
    for C in param_grid["C"]:
        for kernel in param_grid["kernel"]:
            for gamma in param_grid["gamma"]:
                ...
                run_result = {"C": C, "kernel": kernel, "gamma": gamma,
                              "accuracy": acc, "f1": f1}
                results_svm.append(run_result)

                # Log en MLflow
                with mlflow.start_run(run_name=f"SVM_C{C}_K{kernel}_G{gamma}"):
                ...
                print(f"SVM(C={C}, kernel={kernel}, gamma={gamma}) -> Acc: {acc:.4f}, F1: {f1:.4f}")

    # -----------------------------
    # 5) Mejor modelo en test
    # -----------------------------
    best_run = max(results_svm, key=lambda x: x["accuracy"])
    ...
    with mlflow.start_run(run_name="SVM_FINAL_BEST"):
        mlflow.log_param("C", best_run["C"])
        mlflow.log_param("kernel", best_run["kernel"])
        mlflow.log_param("gamma", best_run["gamma"])
        mlflow.log_metric("test_accuracy", acc_test)
        mlflow.log_metric("test_f1", f1_test)
        mlflow.sklearn.log_model(best_model, "svm_final_model")
